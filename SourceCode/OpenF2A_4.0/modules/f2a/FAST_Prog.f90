...
!
!**********************************************************************************************************************************
! Yang: Change the program routine to a subroutine
SUBROUTINE FAST (CTime,NumWTs,H_DTAQWA,StageAQWA,InputFile,PtfmMotIn,PtfmVelIn,PtfmAcceIn,MODE_AQWA)
!call  FAST (TIME,NumWTs,HlfDTAQWA,STAGE,PriFile(iTurbine),LocMot,LocVel,LocAcce,MODE_AQWA)
...
!Yang: Save data of the turbines for the calculation in the next time step
...
! Yang: Add definitions of time and stage
REAL  ::  CTime,H_DTAQWA              ! current simulation time, Half dt in Aqwa.  
REAL :: DT_FAST                       ! Time step in FAST
INTEGER  ::StageAQWA                  ! current simulation stage in AQWA
REAL ::PtfmMotIn(6)                   ! Input platform displacement from AQWA
REAL ::PtfmVelIn(6)                   ! Input platform velocity from AQWA
REAL ::PtfmAcceIn(6)                  ! Input platform acceleration from AQWA  
integer :: NumWTs					  ! Number of wind turbines
integer :: MODE_AQWA				  ! Mode in AQWA. Mode = 99 exit the program, otherwise call FAST normally
! Yang: end of the change


   ! data for restart:
! Yang: change input file length to 1024
CHARACTER(1024)                       :: InputFile                               ! String to hold the intput file name
! Yang: end of change
...
INTEGER(IntKi)                        :: Restart_step                            ! step to start on (for restart) 
      ...
      !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
If (MODE_AQWA == 1) then   ! Yang: judgement for mode in AQWA. call FAST normally only when mode==1 
   ...
   ! Yang: comment out assignment of InputFile 
   !InputFile = ""
   !CheckpointRoot = ""
   
! Yang: Not to check the commandline
   ...
   !ELSE
      Restart_step = 0
      ! Yang: Initilization in FAST
   IF (CTime == 0 .AND. StageAQWA==1)  THEN         ! First invokation of F2A   
      DO i_turb = 1,NumTurbines
         !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         ! initialization
         !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        
        ! Yang: Add InFstFile for initialization 
         CALL FAST_InitializeAll_T( t_initial, i_turb, Turbine(i_turb), ErrStat, ErrMsg, trim(InputFile)  )     
        ! Yang: end of change
      ...
      END DO
   END IF ! Yang: end of judgement for initilization
   !END IF ! Yang: Cencel the judgement of checking computational mode (Restart or normal run) 
   ...
   ! Yang: assign dt of OpenFAST and start loop of time-marching
   DT_FAST = Turbine(1)%p_FAST%DT
   
   TIME_STEP_LOOP:DO n_t_global = NINT((CTime+(StageAQWA-1)*H_DTAQWA)/DT_FAST), NINT((CTime+(StageAQWA-1)*H_DTAQWA)/DT_FAST)+NINT(H_DTAQWA/DT_FAST)-1   ! H_DTAQWA half dt in AQWA 1  
   ! end of change
   ...
   !TIME_STEP_LOOP:  DO n_t_global = Restart_step, Turbine(1)%p_FAST%n_TMax_m1 
      ...
      ! Yang: comment out the below for not checking restart
     ...
     ! Yang: End of change 
      ...
      DO i_turb = 1,NumTurbines
         ...
         !Yang: we do not use F2A to conduct a linarization analysis. 
...
ENDIF ! judgement of mode in AQWA. Time marching will be only performed when AQWAMode is 1. 

! Yang: comment out the linarization code.
  !DO i_turb = 1,NumTurbines
...
!Yang: Comment out the exit procedue to make it be controlled by the AQWA indicator
IF (MODE_AQWA == 99) then
   DO i_turb = 1,NumTurbines
