...
MODULE ElastoDyn
  ...
  ! Yang USE the AQWAINFAST module
   USE AQWAINFAST
  ! YANG:END OF CHANGE
...
!..................................................................................................................................
      ...
      ! Yang: remove this blank subroutine
...
! Yang: chnage it back to OpenFAST 3.5 for coupling with AQWA   
...
!   ENDIF
   ...
   ! Yang: Assign values for tower-base loads.
...
!..................................................................................................................................
   ...
   IF ( p%DOFs%NActvDOF > 0 ) THEN
       ...
       ! Yang: change the input for solutions in order  to not solve platform's DOFs
...
END IF
   ...
   ! Yang: assign solutions
   
   !DO I = 1,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs

   DO I = 1,p%DOFs%NActvDOF-6 ! Loop through all active (enabled) DOFs
      dxdt%QDT(p%DOFs%SrtPS(I+6)) = m%SolnVec(I)    ! dxdt%QDT = m%QD2T
  ...
  ! Yang: Assign solutions of the platform's DOFs
...
!bjj: ERROR CHECKING!!!
   ...
   ! Yang: Assign value for vertical length between tower-base and platform reference point
   LenTwrBs2Ref = p%rZT0zt
   PtfmRef_AQWA = p%PtfmRefzt
   ! Yang: End of change
...
!..................................................................................................................................
   ...
   ! Yang: allocate the matrices without platform's DOFs
      
      CALL AllocAry( m%SolnVec,      p%DOFs%NActvDOF-6,                  'SolnVec',      ErrStat, ErrMsg )
          IF ( ErrStat >= AbortErrLev ) RETURN    
      CALL AllocAry( m%AugMat_pivot, p%DOFs%NActvDOF-6,                  'AugMat_pivot', ErrStat, ErrMsg ) 
          IF ( ErrStat >= AbortErrLev ) RETURN    
      CALL AllocAry( m%AugMat_factor,p%DOFs%NActvDOF-6, p%DOFs%NActvDOF-6, 'AugMat_factor',ErrStat, ErrMsg )
           IF ( ErrStat >= AbortErrLev ) RETURN
...
!! Yang: calculate the added mass (rotational terms) due to the wind turbine  (yy: not used)
!   
!   OffsetVec = OffsetWT(:,iTurbine)
!   LocSinTilt = p%SShftTilt
!   LocCosTilt = p%CShftTilt
!   LocCosPreC = p%CosPreC(1)
!   LocSinPreC = p%SinPreC(1)
!   LocOverHang = p%OverHang
!   LocTwrHt = p%TowerHt
!   LocHubHt = p%HubHt
!   LocTwr2Shaft = InputFileData%Twr2Shft
!   AddedMassBld(3) = 0.0
!   AddedMassHub(3)   = 0.0 
!   AddedMassNac(3)   = 0.0 
!   AddedMassTwr(3)   = 0.0
!   AddedMassTotal(3) = 0.0
!      
!   ! Blades
!   do  K=1,p%NumBl
!       do  J=1,p%BldNodes
!           TempCoord(1) = LocOverHang*p%CShftTilt + p%HubRad + p%RNodes(J)*LocSinPreC
!           TempCoord(2) = p%HubRad + p%RNodes(J)*LocCOSPreC*SIN((K-1)*PI/1.5)
!           TempCoord(3) = LocHubHt + p%HubRad + p%RNodes(J)*LocCOSPreC*COS((K-1)*PI/1.5)
!           TempCoord = TempCoord + OffsetVec
!           AddedMassBld(1) = AddedMassBld(1) + p%BElmntMass(J,K) * ((TempCoord(2)-CoGPtfm(2))**2 +(TempCoord(3)-CoGPtfm(3))**2)
!           AddedMassBld(2) = AddedMassBld(2) + p%BElmntMass(J,K) * ((TempCoord(1)-CoGPtfm(1))**2 +(TempCoord(3)-CoGPtfm(3))**2)
!           AddedMassBld(3) = AddedMassBld(3) + p%BElmntMass(J,K) * ((TempCoord(2)-CoGPtfm(2))**2 +(TempCoord(1)-CoGPtfm(1))**2)
!       enddo ! blade nodes
!   enddo ! blade #
! 
!   ! Hub 
!   TempCoord(1)= LocOverHang*p%CShftTilt
!   TempCoord(2)= 0
!   TempCoord(3)= LocHubHt
!   TempCoord = TempCoord + OffsetVec
!   AddedMassHub(1)   = p%HubMass * ((TempCoord(2)-CoGPtfm(2))**2 +(TempCoord(3)-CoGPtfm(3))**2)
!   AddedMassHub(2)   = p%HubMass * ((TempCoord(1)-CoGPtfm(1))**2 +(TempCoord(3)-CoGPtfm(3))**2)
!   AddedMassHub(3)   = p%HubMass * ((TempCoord(2)-CoGPtfm(2))**2 +(TempCoord(1)-CoGPtfm(1))**2)
!   
!   ! Nacelle
!   TempCoord(1)= p%NacCMxn
!   TempCoord(2)= p%NacCMyn 
!   TempCoord(3)= p%NacCMzn + LocTwrHt
!   TempCoord = TempCoord + OffsetVec
!   AddedMassNac(1)   = p%NacMass * ((TempCoord(2)-CoGPtfm(2))**2 +(TempCoord(3)-CoGPtfm(3))**2)
!   AddedMassNac(2)   = p%NacMass * ((TempCoord(1)-CoGPtfm(1))**2 +(TempCoord(3)-CoGPtfm(3))**2)
!   AddedMassNac(3)   = p%NacMass * ((TempCoord(2)-CoGPtfm(2))**2 +(TempCoord(1)-CoGPtfm(1))**2)
!   
!   ! Tower
!   do J=1,p%TwrNodes
!      TempCoord(1)= 0
!      TempCoord(2)= 0 
!      TempCoord(3)= p%HNodes(J) + p%TowerBsHt       
!      TempCoord = TempCoord + OffsetVec
!      AddedMassTwr(1)   = AddedMassTwr(1) + p%TElmntMass(J) * ((TempCoord(2)-CoGPtfm(2))**2 +(TempCoord(3)-CoGPtfm(3))**2)
!      AddedMassTwr(2)   = AddedMassTwr(2) + p%TElmntMass(J) * ((TempCoord(1)-CoGPtfm(1))**2 +(TempCoord(3)-CoGPtfm(3))**2)
!      AddedMassTwr(3)   = AddedMassTwr(3) + p%TElmntMass(J) * ((TempCoord(2)-CoGPtfm(2))**2 +(TempCoord(1)-CoGPtfm(1))**2)
!   enddo
!   AddedMassTotal = AddedMassTwr + AddedMassNac + AddedMassHub + AddedMassBld
!   AddedWTMass(1:3) = 0
!   AddedWTMass(4:6) = AddedMassTotal(:)
!! Yang: end of change
...
SUBROUTINE SetCoordSy( t, CoordSys, RtHSdat, BlPitch, p, x, ErrStat, ErrMsg )
   ...
   !! Yang'Note: The OpenFAST v4.00 has employed the EulerAngle method for the platform displacement 
            ! transformation for some reason, although this has not been done in previous versions.
            ! I intended to replace the relevant code using our transfromation, like other orientations.
            ! However, this is not allowed, because CoordSys%alpha3 and CoordSys%beta1 are currently used
            ! for velocity update of the platform.
   !
...
! Yang: change matrix transformation method for platform displacement using Euler Angle method.
  call getTransMatEulerInED(TransMat, x%QT(DOF_R), x%QT(DOF_Y), -x%QT(DOF_P))  
  ...
  CoordSys%a3 = TransMat(3,1)*CoordSys%z1 + TransMat(3,2)*CoordSys%z2 + TransMat(3,3)*CoordSys%z3 ! Vector / direction a3 (= -yt from the IEC coord. system).
   ...
   DO J = 1,p%TwrNodes ! Loop through the tower nodes / elements
      ...
      CALL SmllRotTrans( 'tower deflection (ElastoDyn SetCoordSy)', ThetaSS, 0.0_R8Ki, ThetaFA, TransMat, TRIM(Num2LStr(t))//' s', ErrStat2, ErrMsg2 )   ! Get the transformation matrix, TransMat, from tower-base to tower element-fixed coordinate systems.
         ...
         IF (ErrStat >= AbortErrLev) RETURN
   ! Yang: change matrix transformation method for platform displacement using Euler Angle method.
      call getTransMatEulerInED(TransMat, ThetaSS, 0.0_R8Ki, ThetaFA)  
      TransMat = transpose(TransMat)
   ! Yang: end of the change
...
! Yang: change matrix transformation method for tower top displacement using Euler Angle method.
   call getTransMatEulerInED(TransMat, ThetaSS, 0.0_R8Ki, ThetaFA)  
   TransMat = transpose(TransMat)
! Yang: end of the change
...
! Yang: change matrix transformation method for blade node displacement using Euler Angle method.
   call getTransMatEulerInED(TransMat, ThetaLxb, ThetaLyb, 0.0_R8Ki)  
   TransMat = transpose(TransMat)
! Yang: end of the change
...
!   REAL(ReKi)                   :: AngVelEN  (3)                                   ! Angular velocity of the nacelle (body N) in the inertia frame (body E for earth).
   ...
   ! Yang: change it back to OpenFAST 3.5 for coupling with AQWA
   RtHSdat%PAngVelEX(DOF_R   ,0,:) =  CoordSys%z1
   RtHSdat%PAngVelEX(DOF_P   ,0,:) = -CoordSys%z3
   RtHSdat%PAngVelEX(DOF_Y   ,0,:) =  CoordSys%z2
   !RtHSdat%PAngVelEX(DOF_R   ,0,:) =  CoordSys%beta1       ! this is OpenFAST 4.0
   !RtHSdat%PAngVelEX(DOF_P   ,0,:) = -CoordSys%alpha3      ! this is OpenFAST 4.0
   !RtHSdat%PAngVelEX(DOF_Y   ,0,:) =  CoordSys%z2          ! this is OpenFAST 4.0
   ! end of change
...
!.....................................   
   ...
   DO I = 1,p%DOFs%NPYE  ! Loop through all active (enabled) DOFs that contribute to the QD2T-related linear accelerations of the platform center of mass (point Y)
    ...
    ! the below is for openfast 4.0.0
...
!      OtherState%HSSBrTrq  = OtherState%HSSBrTrqC         
      ...
      ! find xdot at t
 ! Yang: Assign values for platform soluations
...
SUBROUTINE FixHSSBrTq ( Integrator, p, x, OtherState, m, ErrStat, ErrMsg )
   ...
   !   using the vector subscript array SrtPS(), after Gauss() has been called:
    ...
    ! Yang: change the input matrix for solution in order not to solve platform's DOFs
...
! Yang: Assign solutions of the platform's DOFs
          ! assign it again for the loop below
   m%QD2T(DOF_Sg) = PtfmAcceAQWA(1)
   m%QD2T(DOF_Sw) = PtfmAcceAQWA(2)
   m%QD2T(DOF_Hv) = PtfmAcceAQWA(3)
   m%QD2T(DOF_R)  = PtfmAcceAQWA(4)
   ...
   DO I = 1,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs
      ...
      !      later if necessary
      ! Yang: change it back because m%SolnVec(1:6) above do not represent platform's DOF
       RqdFrcGeAz = RqdFrcGeAz + m%OgnlGeAzRo(p%DOFs%SrtPS(I))*m%QD2T(p%DOFs%SrtPS(I))  ! {Fb}=[Cba]{Qa}+[Cbb]{Qb}
   ...
   ELSE
      ...
      m%QD2T = 0.0
          ...
          DO I = 1,p%DOFs%NActvDOF-6    ! Loop through all active (enabled) DOFs
             m%QD2T(p%DOFs%SrtPS(I+6)) = m%SolnVec(I)
          ENDDO             ! I - All active (enabled) DOFs     
...
SUBROUTINE FixYawFric ( Integrator, p, x, OtherState, m, ErrStat, ErrMsg )
  ...
  ! Yang: do the similar reduction in matrix for solutions of this subroutine 
...
! Yang: Assign solutions of the platform's DOFs
   ...
   DO I = 1,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs
      ...
      !RqdFrcYaw = RqdFrcYaw + m%OgnlYawRow(p%DOFs%SrtPS(I))*m%SolnVec(I)  ! {Fb}=[Cba]{Qa}+[Cbb]{Qb}  (note that [Cba , Cbb] is the old row, and [Qa;Qb] is a single vector SolVec; %Note this is supposedly= YawFriMz+YawFriMf+DeltaM
       RqdFrcYaw = RqdFrcYaw + m%OgnlYawRow(p%DOFs%SrtPS(I))*m%QD2T(p%DOFs%SrtPS(I))  ! {Fb}=[Cba]{Qa}+[Cbb]{Qb}  (note that [Cba , Cbb] is the old row, and [Qa;Qb] is a single vector SolVec; %Note this is supposedly= YawFriMz+YawFriMf+DeltaM
   ENDDO             ! I - All active (enabled) DOFs
...
! Yang: add a subroutine for Euler angle transformation
SUBROUTINE   getTransMatEulerInED(TransMat, Rot1,Rot2,Rot3)

implicit none

  real(R8Ki) :: TransMat (3,3)       ! Transformation matrix
  real(R8Ki) :: Rot1,Rot2,Rot3            ! Rotations
  real(R8Ki) :: cx,cy,cz             ! Cosines
  real(R8Ki) :: sx,sy,sz             ! Sines
...
END SUBROUTINE getTransMatEulerInED
